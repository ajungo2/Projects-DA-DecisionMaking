...
In this section we are going to decide how to deal with this inconvenient, this is specifically for the case of `AGE`. As previously said, we believe that the 125 age is an error, hence we will discard it by selecting only the observation with value lower then 76 (as 75 is the second highest value).
...
### Available constuction mechanisms 
...
#### Sex variable {.tabset .tabset-fade .tabset-pills}
...
We will now move on to the other variables above mentioned, so that instead of having multiple dummy variables, we have factor variables with multiple levels. 
...
5 = the purpose for the credit was to increase the level of eductation 
...
data %>% 
  ...
  labs(x = "Type of purpose", fill="Purpose",  
       title = "Purpose Variable ranking by type",  
       y = "Quantity [units]")
...
#### Property {.tabset .tabset-fade .tabset-pills}

Now, we will create the property variable. 

We will start by looking at if the two variables that we want to use (namely, `REAL_ESTATE` and `PROP_UNKN_NONE`) are connected and hence it could makes sense to put them together. 
...
*Conclusion:* As, We can see that the two variables are statistically significantly associated, as the p-value is really low, almost equal to 0, and hence is lower than the considered significance level of alpha = 5%.
...
#### Residence {.tabset .tabset-fade .tabset-pills}

Now let's look at the second variable that we wish to know if it is needed to be created using the `RENT` and `OWN_RES` variables to describe whether a person has a residence or not. 
...
##### Types of residence by Response
...
### Significance and correlation test

In the next step we are going to perform significance and correlation tests to decide what to include.
