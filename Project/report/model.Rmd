...
data_sel %>% dplyr::select(RESPONSE) %>% group_by(RESPONSE) %>% count()
...
```{r pruning cart, echo=FALSE, fig.asp=5}
...
confusionMatrix(as.factor(mda.pred), as.factor(TestData$RESPONSE))
...
confusionMatrix(as.factor(fda.pred), as.factor(TestData$RESPONSE))
